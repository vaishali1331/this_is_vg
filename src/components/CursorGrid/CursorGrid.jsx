// React Bits — Cursor Grid (JS + CSS variant)
// Source: https://reactbits.dev/animations/cursor-grid
// Installed via: npx jsrepo@latest add https://reactbits.dev/r/CursorGrid-JS-CSS
// (fetched directly from the registry endpoint above; kept as-authored,
// only this header + the JSDoc props block below were added.)
import { useRef, useEffect } from 'react';
import './CursorGrid.css';

// Maps normalized distance-from-cursor (0 = center, 1 = edge of radius)
// to brightness, so cells fade in with different curves near the pointer.
...
/**
 * Canvas grid whose cells light up around the cursor with configurable
 * radius, falloff curve, and optional click pulses.
 *
 * @param {Object} props
 * @param {number} [props.cellSize=70] - Grid cell size in px.
 * @param {string} [props.color='#D946EF'] - Hex color used for cell glow/fill.
 * @param {number} [props.radius=140] - Px radius around the cursor that lights up cells.
 * @param {'linear'|'smooth'|'sharp'} [props.falloff='smooth'] - Brightness curve across the radius.
 * @param {number} [props.holdTime=400] - Ms a lit cell stays at full brightness before fading.
 * @param {number} [props.fadeDuration=800] - Ms it takes a cell to fade out once it starts fading.
 * @param {number} [props.lineWidth=1.2] - Stroke width of each cell's outline glow.
 * @param {number} [props.maxOpacity=1] - Peak opacity a cell can reach.
 * @param {number} [props.fillOpacity=0] - Opacity of the cell's fill (0 = outline-only).
 * @param {number} [props.gridOpacity=0] - Opacity of a faint always-visible static grid.
 * @param {number} [props.cellRadius=0] - Corner radius for cell squares (rounded cells).
 * @param {boolean} [props.clickPulse=true] - Emit an expanding ring of light on click.
 * @param {number} [props.pulseSpeed=600] - Px/second the click-pulse ring expands at.
 * @param {string} [props.className] - Extra class name(s) for the outer container.
 */
const CursorGrid = ({
  cellSize = 70,
  color = '#D946EF',
